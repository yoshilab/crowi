module.exports = function(crowi) {
  ...
  var USER_PUBLIC_FIELDS = '_id image googleId name username email introduction status lang createdAt admin' // TODO: どこか別の場所へ...
  var LANG_EN = 'en'
  ...
  userSchema.methods.updateGitHubId = function(githubId, callback) {
  ...
  userSchema.methods.deleteGitHubId = function(callback) {
    return this.updateGitHubId(null, callback)
  }
  ...
  userSchema.statics.findAllUsers = function(option) {
    ...
    var fields = option.fields || USER_PUBLIC_FIELDS
  ...
  userSchema.statics.findUserByUsername = function(username) {
    var User = this
    return new Promise(function(resolve, reject) {
      User.findOne({ username }, function(err, userData) {
        if (err) {
          return reject(err)
        }
  ...
  userSchema.statics.findUserByApiToken = function(apiToken) {
    ...
    return new Promise(function(resolve, reject) {
      self.findOne({ apiToken }, function(err, userData) {
        if (err) {
          return reject(err)
        } else {
          return resolve(userData)
  ...
  userSchema.statics.findUserByGitHubId = function(githubId, callback) {
    this.findOne({ githubId }, function(err, userData) {
      callback(err, userData)
    })
  }
  ...
  userSchema.statics.findUserByEmailAndPassword = function(email, password, callback) {
    ...
    this.findOne({ email, password: hashedPassword }, function(err, userData) {
      callback(err, userData)
    })
  }
  ...
  userSchema.statics.isRegisterableUsername = function(username, callback) {
    ...
    this.findOne({ username }, function(err, userData) {
      if (userData) {
