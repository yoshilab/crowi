module.exports = function(crowi) {
  ...
  pageSchema.statics.findPage = function(path, userData, revisionId, ignoreNotFound) {
    ...
    return new Promise(function(resolve, reject) {
      self.findOne({ path }, function(err, pageData) {
        if (err) {
          return reject(err)
        }
  ...
  pageSchema.statics.findPageByPath = function(path) {
    ...
    return new Promise(function(resolve, reject) {
      Page.findOne({ path }, function(err, pageData) {
  ...
  pageSchema.statics.findListByPageIds = function(ids, options) {
    ...
    return new Promise(function(resolve, reject) {
      Page.find({ _id: { $in: ids }, grant: GRANT_PUBLIC })
        ...
        .skip(offset)
        .limit(limit)
        .populate([{ path: 'creator', model: 'User', select: User.USER_PUBLIC_FIELDS }, { path: 'revision', model: 'Revision' }])
    ...
    return new Promise(function(resolve, reject) {
      Page.findOne({ path }, function(err, pageData) {
        ...
        newPage.save(function(err, newPage) {
          ...
          var newRevision = Revision.prepareRevision(newPage, body, user, { format })
